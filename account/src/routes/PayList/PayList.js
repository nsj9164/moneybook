...
function PayList() {
    ...
    useEffect(() => {
        if(payListStatus === 'succeeded' && payList.length > 0) {
            setTempData(payList.map(item => ({ ...item, isDisabled:false, isModified:false })));
        } else {
    ...
    useEffect(() => {
        ...
        // 합계 count
        let sumPrice1 = tempData.filter(item => item.price1 && unformatNumber(item.price1) > 0).reduce((sum, item) => sum + parseInt(unformatNumber( item.price1)), 0);
        setExpense(sumPrice1.toLocaleString('ko-KR'));
        
        let sumPrice2 = tempData.filter(item => item.price2 && unformatNumber(item.price2) > 0).reduce((sum, item) => sum + parseInt(unformatNumber( item.price2)), 0);
    ...
    const handleUpdate = (e, id, key) => {
        ...
        setTempData(prevData => prevData.map((item) => {
            console.log(item.id)
            return item.id === id ? { ...item, [key]: newItem, isModified: true } : item
        }));
    }
        ...
        if(item.isDisabled) {
            const newData = tempData.map((i) => {
                if(item.id === i.id) {
                    ...
                    return {...i, id:`${date}-${tempId}`, date, price1: 0, price2: 0, isDisabled: false, isNew: true}
                } else {
                    return i
                }
            })
    ...
    // 데이터 복사
    const handleCopy = () => {
        const copyCheckedList = tempData.filter(item => checkedItems.includes(item.id))
                                    .map(item => ({ ...item }));
        ...
        setTempData([...tempData.slice(0,tempData.length-1), ...copyCheckedList, ...tempData.slice(-1)]);
        setCheckedItems([]);
    }
    ...
    const handleKeyDown = (e, i, col) => {
        ...
        if(col.includes("price")) {
            ...
            if((/\d/.test(e.key) && e.target.innerText.length > 12) || blockedRegex.test(e.key)) {
                e.preventDefault();
            }
        }
    }
    ...
    const handleInput = (e, col) => {
        if(col.includes('price')) {
            ...
            if(e.target.innerText !== "\n") {
                ...
                let adjustedStartOffset = (price === formattedValue && !/[ㄱ-ㅎ가-힣]/.test(value)) ? cursor.startOffset : cursor.startOffset + diffLength;

                restoreCursor(e.target, adjustedStartOffset >= 0 ? adjustedStartOffset : 0);
            }
        }
    }
    ...
    return (
        <div className="payList_contents">
            <div className="date_wrap">
                <Button variant="outline-dark" className="btn_month_prev" onClick={()=>handleDate('prev')}>&lt;</Button>
